...
class SystersUserTestCase(TestCase):
    ...
    def test_leave_groups(self):
        ...
        community_moderator_group = Group.objects.get(name="Baz: Community Moderator")
        self.systers_user.join_group(community_moderator_group)
        self.assertSequenceEqual(self.systers_user.user.groups.all(),
                                 [community_moderator_group])
        self.systers_user.leave_groups(name)
        self.assertSequenceEqual(self.systers_user.user.groups.all(), [])
        other_name = "Foo"
        create_groups(other_name, groups_templates)
        admin_group = Group.objects.get(name="Foo: Community Leader")
        self.systers_user.join_group(admin_group)
        self.systers_user.join_group(community_moderator_group)
        self.assertCountEqual(list(self.systers_user.user.groups.all()),
                              [community_moderator_group, admin_group])
        self.systers_user.leave_groups(name)
        self.assertSequenceEqual(self.systers_user.user.groups.all(),
                                 [admin_group])
    ...
    def test_leave_community(self):
        ...
        community.save()
        self.assertTrue(bar_systers_user.is_member(community))
        community_moderator_group = Group.objects.get(name="Foo: Community Moderator")
        bar_systers_user.join_group(community_moderator_group)
